...
//            g.drawLine(x, y, x, y);
//        }
        CodeCaptcha codeCaptcha = new CodeCaptcha();
        String code = codeCaptcha.getRondom();



        // 将生成的随机字符串写入session
       // request.getSession().setAttribute(LOGIN_VERCODE_KEY, rand);
        request.getSession().setAttribute(VERCODE_KEY, code);
//        //图象生效
//        g.dispose();
        //输出图象到页面
        ImageIO.write(codeCaptcha.getImage(code), "JPEG", response.getOutputStream());
    }
